<?php
    try {
        ...
        //      }
        //  }
        if(isset($_POST["newName"]) && isset($_POST["newPrice"])) {
            if(empty($_POST["newName"]) || empty($_POST["newPrice"])) {
                $error = "Valid sefer information must be submitted";
            } else {
                
               $query1 = "INSERT INTO seforim(name,price) VALUES(:theName, :thePrice)";
               $statement1 = $db->prepare($query1);
               $statement1->bindValue("theName", $_POST["newName"]);
               $statement1->bindValue("thePrice", $_POST["newPrice"]);
               $statement1->execute();
